  ...
  if (needAuth) {
    ...
    if (token) config.headers = { authorization: token };
      ...
      throw new AuthenticationError(
        "Request needs to carry auth, but no auth token found"
      );
  }
...
function processRequestError(err) {
  ...
  if (response) {
    // received a non-2xx response from server
  ...
  if (request) {
    ...
    throw new HTTPError(
      "The request was made, but the server did not respond for an unknown reason"
